...
- 确保学号和用户名不重复
- 用户名采用统一格式: stu + 学号
- 文件名包含班级详细信息，便于批量导入时自动关联班级
- 生成的 Excel 文件可直接用于管理员批量注册学生接口
  - 标准导入: /api/eduagentx/admin/register/student/excel
  - 自动处理班级: /api/eduagentx/admin/register/student/excel/with-class
"""
...
def generate_students(major_code, grade, class_count, students_per_class=30):
    ...
    return all_students, major_name
...
def main():
    """主函数"""
    print("欢迎使用学生信息生成脚本（改进版V2）！")
    print("注意：此版本支持通过文件名自动关联班级信息")
    ...
    # 生成学生数据
    students, major_name = generate_students(major_code, grade, class_count, students_per_class)
    ...
    # 文件名格式：专业名称_年级级_时间戳.xlsx
    file_name = f"{major_name}_{grade}级_{timestamp}.xlsx"
    ...
    print("\n导入提示:")
    print("1. 使用标准导入接口: /api/eduagentx/admin/register/student/excel")
    print("2. 使用自动处理班级的接口: /api/eduagentx/admin/register/student/excel/with-class")
    print("   - 后者可以自动从文件名中解析班级信息，并处理班级关联")
