...
"""
学生信息生成脚本（班级版）
按班级生成学生信息并创建Excel文件，每个班级一个文件
- 确保学号和用户名不重复
- 用户名采用统一格式: stu + 学号
- 文件名按格式"专业名称_年级级_班号班_时间戳.xlsx"命名
- 支持管理员批量注册学生并自动关联班级接口 (/api/eduagentx/admin/register/student/excel/with-class)
"""
...
def generate_class_code(major_code, grade, class_num):
    """生成班级代码: 专业代码(2) + 年级(2) + 班级序号(2)"""
    return f"{major_code}{grade}{class_num:02d}"

def generate_class_name(major_name, grade, class_num):
    """生成班级名称: 专业名称 + 年级 + 班级序号 + 班"""
    return f"{major_name}{grade}-{class_num}班"

def generate_students_for_class(major_code, grade, class_num, start_counter, students_count=30):
    """
    为指定的班级生成学生数据
    """
    students = []
    ...
    class_name = generate_class_name(major_name, grade, class_num)
    college = get_college_for_major(major_code)
    
    for i in range(students_count):
        student_counter = start_counter + i
        ...
        student = {
            ...
            'classCode': class_name,  # 使用班级名称作为classCode
            'college': college,
            'school': "中国矿业大学",
    ...
    return students, class_name, major_name
...
def main():
    """主函数"""
    print("欢迎使用学生信息生成脚本（班级版）！")
    print("本脚本将按班级生成学生信息，每个班级一个Excel文件")
    ...
    total_students = 0
    student_counter_start = 1  # 学号计数器起始值
    
    # 为每个班级生成一个文件
    for class_num in range(1, class_count + 1):
        # 生成该班级的学生数据
        students, class_name, major_name = generate_students_for_class(
            major_code, 
            grade, 
            class_num, 
            student_counter_start, 
            students_per_class
        )
        
        # 更新学号计数器起始值
        student_counter_start += students_per_class
        
        # 文件名格式：专业名称_年级级_班号班_时间戳.xlsx
        file_name = f"{major_name}_{grade}级_{class_num}班_{timestamp}.xlsx"
    ...
    print(f"\n总计生成 {total_students} 名学生信息，共 {class_count} 个班级！")
    print(f"专业: {MAJOR_INFO[major_code]}, 年级: {grade}级")
    print("\n导入提示:")
    print("请使用自动处理班级的接口: /api/eduagentx/admin/register/student/excel/with-class")
    print("该接口会自动从文件名中解析班级信息，并处理班级关联")
